...
/datum/vore_look/export_panel/ui_interact(mob/user, datum/tgui/ui)
...
/*/datum/vore_look/export_panel/ui_fallback(payload) <-- Hopefully nothing important...?
	if(..())
		return TRUE*/
...
/datum/vore_look/export_panel/ui_act(action, params)
	if(..())
		return TRUE

/datum/vore_look/export_panel/ui_data(mob/user)
	var/list/data = list()
